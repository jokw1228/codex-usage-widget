...
- Codex 기본 한도의 잔여량을 표시합니다. Spark 등 모델별 별도 한도와 혼합하지 않습니다.
- 계정 응답에 포함된 별도 한도는 탭으로 각각 확인할 수 있습니다. 새로고침 중에도 선택한 탭을 유지합니다.
...
- 옵션 버튼을 누르면 카드가 뒤집히고, 뒷면에서 위젯 투명도를 조절하고 저장할 수 있습니다.
- 창 가장자리를 잡아 위젯 크기를 조절할 수 있습니다.
...
- 위젯 이동: 카드 아무 곳이나 드래그
- 크기 조절: 창 가장자리나 모서리 드래그
- 새로고침: 우측 상단 버튼
- 한도 선택: 상단 탭. 별도 한도가 있을 때 표시됩니다.
- 닫기: 우측 상단 `×` 버튼
- 옵션: 우측 상단 톱니 버튼
- 투명도 조절: 뒤집힌 옵션 화면의 슬라이더
- 설정 초기화: 뒤집힌 옵션 화면의 `설정 초기화` 버튼
- 보이기/숨기기: `Ctrl+Alt+U`
- 숨기기: `Esc`

## 설정 저장

투명도, 창 위치, 창 크기 같은 사용자 설정은 Windows 사용자 데이터 경로에 저장합니다. 옵션 화면에서 기본 투명도와 기본 크기로 초기화할 수 있습니다.
...
표시하는 값은 정확한 잔여 토큰 수가 아니라 한도별 잔여 비율입니다. 요금제 이름으로 한도를 추측하지 않고, 현재 계정의 Codex app-server 응답에 있는 종류와 기간을 표시합니다. 일반 ChatGPT 대화 등 이 응답에 포함되지 않는 사용량까지 조회하는 도구는 아닙니다. 정보가 없는 값은 `--%`로 표시하며, 서로 다른 한도의 비율을 합산하지 않습니다.
